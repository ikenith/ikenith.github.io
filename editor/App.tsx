...
import { LogType, ActivityLog, Draft, DraftData, Post, Toast } from './types';
...
const App: React.FC = () => {
  ...
  const [repoPath, setRepoPath] = useState(() => localStorage.getItem(LOCAL_STORAGE_REPO) || 'username/repo.github.io');
  ...
  useEffect(() => {
    localStorage.setItem(LOCAL_STORAGE_REPO, repoPath);
  }, [repoPath]);
  ...
  const handleSaveDraft = (name: string) => {
    ...
    if (currentDraftId) {
      const updated = drafts.map(d => d.id === currentDraftId 
        ? { ...d, name, data, updatedAt: new Date().toISOString() } 
        : d
      );
  ...
  const loadDraft = (draft: Draft) => {
    ...
    setCroppedImageData(data.croppedImageData);
    setFilename(data.filename);
    setIsEditing(data.isEditing);
    setCurrentPostSha(data.postSha);
    setCurrentImageSha(data.imageSha);
  ...
  const clearForm = () => {
    ...
    setFilename('');
    setIsEditing(false);
    setCurrentPostSha(null);
    setCurrentImageSha(null);
  ...
  const publishPost = async () => {
    ...
    try {
      ...
      if (imageOption === 'upload' && croppedImageData) {
        ...
        const imgResponse = await fetch(`https://api.github.com/repos/${repo}/contents/${imagePath}`, {
          method: 'PUT',
          headers: {
          ...
          body: JSON.stringify({
            message: `Update image for ${title}`,
            content: base64Data,
            sha: finalImageSha || undefined
          })
        });
      ...
      const response = await fetch(`https://api.github.com/repos/${repo}/contents/${filePath}`, {
        ...
        body: JSON.stringify({
          message: `${isEditing ? 'Update' : 'Add'} post: ${title}`,
          content: btoa(unescape(encodeURIComponent(fullContent))),
          sha: currentPostSha || undefined
        })
      });
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="fixed top-20 right-4 z-[100] flex flex-col gap-2 w-full max-w-sm pointer-events-none">
        {toasts.map(t => (
          <div key={t.id} className={`p-4 rounded-xl border shadow-2xl flex justify-between items-center pointer-events-auto backdrop-blur-md animate-in slide-in-from-right duration-300 ${
            t.type === 'success' ? 'bg-emerald-500/10 border-emerald-500 text-emerald-400' :
            t.type === 'error' ? 'bg-rose-500/10 border-rose-500 text-rose-400' :
            t.type === 'warning' ? 'bg-amber-500/10 border-amber-500 text-amber-400' :
            'bg-blue-500/10 border-blue-500 text-blue-400'
          }`}>
            <span className="font-medium">{t.message}</span>
            <button onClick={() => setToasts(prev => prev.filter(x => x.id !== t.id))} className="ml-4 opacity-70 hover:opacity-100">
              <Icons.Close size={18} />
            </button>
          </div>
        ))}
      ...
      <nav className="fixed top-0 left-0 right-0 h-16 bg-zinc-900/80 backdrop-blur-xl border-b border-zinc-800 z-50 flex items-center px-6">
        <div className="flex items-center gap-3 font-bold text-xl tracking-tight text-white cursor-pointer" onClick={() => window.location.reload()}>
          <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center text-white">
            <Icons.Feather size={20} />
          </div>
          <span>The Roots</span>
        </div>
        <div className="flex-1 flex justify-center gap-8 text-sm font-medium text-zinc-400">
          <a href="#" className="hover:text-white transition-colors">Dashboard</a>
          <a href="#" className="hover:text-white transition-colors">Writings</a>
          <a href="#" className="hover:text-white transition-colors">Analytics</a>
        </div>
        <div className="flex items-center gap-3">
          <button 
            onClick={() => setIsSidebarOpen(true)}
            className="p-2 hover:bg-zinc-800 rounded-lg text-zinc-400 relative"
          >
            <Icons.Bell size={20} />
      ...
      <Sidebar 
        ...
        onClear={() => {
          setLogs([]);
          localStorage.removeItem(LOCAL_STORAGE_LOGS);
        }}
      />
      ...
      <main className="mt-16 flex-1 flex flex-col md:flex-row p-6 gap-6 max-w-[1600px] mx-auto w-full">
        ...
        <div className={`flex-1 flex flex-col gap-6 transition-all duration-300 ${isPreviewVisible ? 'md:w-1/2' : 'w-full'}`}>
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden shadow-xl flex flex-col">
            <div className="p-4 border-b border-zinc-800 flex justify-between items-center bg-zinc-900/50">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-zinc-800 rounded-xl flex items-center justify-center text-blue-400 shadow-inner">
                  <Icons.Edit size={22} />
                </div>
                <div>
              ...
              <div className="flex items-center gap-2">
                <button 
                  onClick={() => setModalOpen('save-draft')}
                  className="px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-200 rounded-lg text-xs font-semibold transition-all flex items-center gap-2"
                >
                  <Icons.Save size={14} /> Save Draft
                </button>
                <button 
                   onClick={() => setModalOpen('load-draft')}
                   className="px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-200 rounded-lg text-xs font-semibold transition-all flex items-center gap-2"
                >
                  <Icons.Folder size={14} /> Load Draft
                </button>
                <button 
                  onClick={() => setIsPreviewVisible(!isPreviewVisible)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all flex items-center gap-2 ${isPreviewVisible ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'bg-zinc-800 text-zinc-400'}`}
                >
            ...
            <div className="p-6 space-y-6 overflow-y-auto custom-scrollbar">
              ...
              <div className="bg-zinc-950/50 border border-zinc-800 p-4 rounded-2xl space-y-4">
                <div className="flex items-center gap-2 mb-2">
                   ...
                   <h3 className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Connection Setup</h3>
                </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative group">
                      <input 
                        ...
                        placeholder="ghp_..."
                        className="w-full bg-zinc-900 border border-zinc-800 text-zinc-200 rounded-xl px-4 py-2 text-sm focus:border-blue-500/50 outline-none transition-all font-mono"
                      />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <input 
                    type="text"
                    value={title}
                    onChange={(e) => updateFilenameFromTitle(e.target.value)}
                    placeholder="Enter post title..."
                    className="w-full bg-zinc-950 border border-zinc-800 text-zinc-200 rounded-xl px-4 py-3 text-sm focus:border-blue-500/50 outline-none transition-all"
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full bg-zinc-950 border border-zinc-800 text-zinc-200 rounded-xl px-4 py-3 text-sm focus:border-blue-500/50 outline-none transition-all [color-scheme:dark]"
                  />
                </div>
              </div>
                  ...
                  <div className="flex bg-zinc-950 p-1 rounded-lg border border-zinc-800">
                    {(['upload', 'url', 'none'] as const).map(opt => (
                      <button
                        ...
                        className={`px-3 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider transition-all ${imageOption === opt ? 'bg-zinc-800 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}
                      >
                        {opt}
                      </button>
                    ))}
                  ...
                  <div className="space-y-4">
                    {!croppedImageData ? (
                      <div 
                        ...
                        className="w-full h-40 border-2 border-dashed border-zinc-800 rounded-2xl flex flex-col items-center justify-center gap-3 cursor-pointer hover:border-blue-500/50 hover:bg-zinc-800/20 transition-all group"
                      >
                        <div className="w-12 h-12 rounded-full bg-zinc-800 flex items-center justify-center text-zinc-500 group-hover:text-blue-500 transition-colors">
              ...
              <div className="space-y-2">
                ...
                <textarea 
                  ...
                  className="w-full h-[500px] bg-zinc-950 border border-zinc-800 text-zinc-300 rounded-xl px-5 py-5 text-sm font-mono focus:border-blue-500/30 outline-none transition-all resize-none custom-scrollbar leading-relaxed"
                />
              </div>
            </div>
            ...
            <div className="p-4 bg-zinc-950/50 border-t border-zinc-800 flex justify-between items-center">
               ...
               <div className="flex gap-3">
                 <button onClick={clearForm} className="px-4 py-2 hover:bg-zinc-800 text-zinc-400 rounded-xl text-xs font-bold transition-all uppercase tracking-wider">Clear</button>
                 <button 
                  onClick={publishPost}
                  className="px-6 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-bold transition-all shadow-lg shadow-blue-600/20 active:scale-95 flex items-center gap-2 uppercase tracking-wider"
                 >
                   <Icons.CloudUpload size={16} /> {isEditing ? 'Update Post' : 'Deploy to Website'}
                 </button>
               </div>
            </div>
          </div>
        ...
        {/* Preview Side */}
        {isPreviewVisible && (
          <div className="flex-1 min-w-0 flex flex-col animate-in fade-in slide-in-from-right-4 duration-500">
            <Preview 
              content={content} 
              title={title} 
              date={date} 
              image={imageOption === 'upload' ? croppedImageData : imageUrl} 
            />
          </div>
        )}
      </main>
      ...
      <Modals 
        ...
        drafts={drafts}
        onDeleteDraft={(id) => saveDraftsToStorage(drafts.filter(d => d.id !== id))}
        onSetImage={(data) => {
